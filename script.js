function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    const words = htmlContent.replace(/<[^>]+>/g, ' ').split(/\s+/);
  
    let modifiedHtmlContent = '';
    let plainTextPointer = 0;
    for (const word of words) {
      const wordStart = modifiedHtmlContent.length;
      const wordEnd = wordStart + word.length;
  
      const currentHighlight = plainTextPositions.find(
        (pos) => pos.start <= plainTextPointer && pos.end >= plainTextPointer
      );
  
      if (currentHighlight) {
        modifiedHtmlContent += `<mark>${word}</mark>`;
      } else {
        modifiedHtmlContent += word;
      }
      modifiedHtmlContent += ' ';
      plainTextPointer += word.length + 1; // +1 to account for the space after the word
    }
  ...
  document.getElementById('content').innerHTML = modifiedHtmlContent;